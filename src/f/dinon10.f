      SUBROUTINE DINON10(NEQ,UL,DUL,UTL,NNO,
...
C     DIS_ECRO_CINE3D
C
C     RELATION DE COMPORTEMENT CINEMATIQUE 3D EXACTE (RETURN MAPPING)
C
C     f = |F - X| - Fy
C     .      .         .   .
C     F = Ke.Uel = Ke.(U - Upl)
...
C     VECTEUR FORCE F- = X- + Ke * ( U- - Upl- )
!       WRITE(6,*) "U-    = ",VARIMO(1:6)
      CALL DCOPY(NBCOMP, VARIMO(1), 1, FM, 1)
!       WRITE(6,*) "FM    = ",FM
!       WRITE(6,*) "KT    = ",KT
      CALL DSCAL(NBCOMP, -KT, FM, 1)
!       WRITE(6,*) "FM    = ",FM
!       WRITE(6,*) "ULEL  = ",ULEL
      CALL DAXPY(NBCOMP, KT, ULEL, 1, FM, 1)
      CALL DAXPY(NBCOMP, 1.0D0, VARIMO(15), 1, FM, 1)
      WRITE(6,*) "FM    = ",FM(1:2)

C     VECTEUR FORCE F+ = X+ + Ke * ( U+ - Upl+ )
C     APPROXIMATION: Upl+ = Upl- ET X+ = X-
      CALL DCOPY(NBCOMP, VARIMO(1), 1, FP, 1)
!       WRITE(6,*) "FP    = ",FP
      CALL DSCAL(NBCOMP, -KT, FP, 1)
!       WRITE(6,*) "FP    = ",FP
!       WRITE(6,*) "UTLEL = ",UTLEL
      CALL DAXPY(NBCOMP, KT, UTLEL, 1, FP, 1)
      CALL DAXPY(NBCOMP, 1.0D0, VARIMO(15), 1, FP, 1)
...
! C     CENTRE DE LA SURFACE DE CHARGE X+ = 2/3 * H * A * C+
! C     APPROXIMATION: C+ = C-
!       CALL DCOPY(NBCOMP, VARIMO(15), 1, XP, 1)
!       WRITE(6,*) "XP    = ",XP(1:2)," (approximation X+ = X-)"

C     VALEUR DE LA FONCTION DE SURFACE DE CHARGE
C     f = |F+ - X+| - Fy = |F+ - X-| - Fy (approx) 
...
C     TEST DE COMPORTEMENT ELASTIQUE
      IF ( NORM .LE. FY ) THEN
         ...
         RETURN
      ENDIF
      WRITE(6,*) "||F - X|| > Fy : PLASTIC BEHAVIOR"
...
!       WRITE(6,*) "VTMP  = ",VTMP
      CALL DAXPY(NBCOMP, KT, DULEL, 1, VTMP, 1)
!       WRITE(6,*) "VTMP  = ",VTMP
      CALL DAXPY(NBCOMP, -1.0D0, XM, 1, VTMP, 1)
!       WRITE(6,*) "VTMP  = ",VTMP
      NORM = DNRM2(NBCOMP, VTMP, 1)
...
!       WRITE(6,*) "VTMP  = ",VTMP(1:2)
      CALL DAXPY(NBCOMP, KT, VTMP, 1, FP, 1)
!       WRITE(6,*) "FP  = ",FP(1:2)
